...
class Method(object, metaclass=abc.ABCMeta):
    ...
    def metric(self, predictions, labels, y_info):
        ...
        if self.is_regression:
            ...
            auc = skm.roc_auc_score(labels, predictions[:, 1], labels=y_info['classes']) if len(
                np.unique(labels)) == 2 else float("nan")
            return (accuracy, avg_recall, avg_precision, f1_score, log_loss, auc), ("Accuracy", "Avg_Recall",
                                                                                    "Avg_Precision", "F1", "LogLoss",
                                                                                    "AUC")
